...
import "fraxlend/src/contracts//FraxlendPairConstants.sol";
import "fraxlend/src/contracts/libraries/VaultAccount.sol";
import "fraxlend/src/contracts/libraries/SafeERC20.sol";
import "fraxlend/src/contracts/interfaces/IERC4626.sol";
import "fraxlend/src/contracts/interfaces/IFraxlendWhitelist.sol";
import "fraxlend/src/contracts/interfaces/IRateCalculator.sol";
import "fraxlend/src/contracts/interfaces/ISwapper.sol";
...
abstract contract FraxlendPairCore is
    ...
    ReentrancyGuard
{
    ...
    constructor(
...

        {
            ...
            if (_maxLTV >= LTV_PRECISION && !_isBorrowerWhitelistActive)
                revert BorrowerWhitelistRequired();
            maxLTV = _maxLTV;

            // Oracle Settings
            {
                IFraxlendWhitelist _fraxlendWhitelist = IFraxlendWhitelist(
        ...
        returns (bool)
    {
        ...
        uint256 _borrowerAmount = totalBorrow.toAmount(
        ...
        uint256 _ltv = (((_borrowerAmount * _exchangeRate) /
            EXCHANGE_PRECISION) * LTV_PRECISION) / _collateralAmount;
        return _ltv <= maxLTV;
    }
    ...
    modifier approvedLender(address _receiver) {
        if (
            ...
            (!approvedLenders[msg.sender] || !approvedLenders[_receiver])
        ) {
        ...
        if (_totalBorrow.shares == 0 || paused()) {
            ...
            if (
                ...
                if (_currentRateInfo.feeToProtocolRate > 0) {
                    ...
                    _feesShare =
                        (_feesAmount * _totalAsset.shares) /
                        (_totalAsset.amount - _feesAmount);
    ...
    function _updateExchangeRate() internal returns (uint256 _exchangeRate) {
        ...
        if (oracleMultiply != address(0)) {
            (, int256 _answer, , , ) = AggregatorV3Interface(oracleMultiply)
                .latestRoundData();
            if (_answer <= 0) {
        ...
        if (oracleDivide != address(0)) {
            (, int256 _answer, , , ) = AggregatorV3Interface(oracleDivide)
                .latestRoundData();
            if (_answer <= 0) {
    ...
    function repayAsset(uint256 _shares, address _borrower)
        external
        nonReentrant
        ...
        returns (uint256 _collateralForLiquidator)
    {
        if (block.timestamp > _deadline)
            revert PastDeadline(block.timestamp, _deadline);
        ...
        int256 _leftoverCollateral;
        {
            ...
            uint256 _liquidationAmountInCollateralUnits = ((_totalBorrow
                .toAmount(_sharesToLiquidate, false) * _exchangeRate) /
                EXCHANGE_PRECISION);
        ...
        uint128 _sharesToAdjust;
        {
            ...
            if (_leftoverCollateral <= 0) {
                ...
                if (_sharesToAdjust > 0) {
                    // Write off bad debt
                    _amountToAdjust = (
                        _totalBorrow.toAmount(_sharesToAdjust, false)
                    ).toUint128();
        ...
        returns (uint256 _totalCollateralBalance)
    {
        ...
        uint256 _borrowShares = _borrowAsset(
            _borrowAmount.toUint128(),
            address(this)
        );
        ...
        uint256 _initialCollateralBalance = _collateralContract.balanceOf(
            address(this)
        );
        ...
        uint256 _finalCollateralBalance = _collateralContract.balanceOf(
            address(this)
        );

        // Note: VIOLATES CHECKS-EFFECTS-INTERACTION pattern, make sure function is NONREENTRANT
        // Effects: bookkeeping & write to state
        uint256 _amountCollateralOut = _finalCollateralBalance -
            _initialCollateralBalance;
